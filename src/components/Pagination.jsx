...
const Pagination = ({ queries }) => {
  ...
  const paginationOnClickHandler = (startIndex) => {
    setPage(startIndex);
    ...
    <div className="flex flex-col items-center py-14 max-w-[700px]">
      <div className="relative text-[#4285f4]">
        {queries.previousPage && (
          <div
            className="absolute left-[-30px] md:left-[-50px] top-[10px]"
            onClick={() =>
              paginationOnClickHandler(queries.previousPage[0].startIndex)
            }
          >
        ...
        <img className="w-[250px] md:w-[300px]" src={Logo} alt="Logo" />
        {queries.nextPage && (
          <div
            className="absolute right-[-30px] md:right-[-50px] top-[10px]"
            onClick={() =>
              paginationOnClickHandler(queries.nextPage[0].startIndex)
            }
          >
      ...
      <div className="flex gap-3 text-[#4285f4] text-sm">
        {pagination.map((p) => (
          <span
            key={p.page}
            onClick={() => paginationOnClickHandler(p.startIndex)}
            className={`1cursor-pointer ${
